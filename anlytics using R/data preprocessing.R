...
# After transformation Age skewness reduced from 0.74 to 0.37

# convert trial from int to factor
FTrials$trial <- factor(FTrials$trial, levels = c(0,1))

### Task 2: prepare data to feed into model
# like convert categorical features to numeric

# filter data for each group
FTrials_control <- filter(FTrials, group == '1') # Control
FTrials_control <- FTrials_control[-9] # drop feature "group"
str(FTrials_control)
FTrials_test <- filter(FTrials, group == '2') # test
FTrials_test <- FTrials_test[-9] # drop feature "group"
str(FTrials_test)

# save in csv file to build models
write.csv(FTrials_control, 'control_trial_results_map.csv')
write.csv(FTrials_test, 'test_trial_results_map.csv')
